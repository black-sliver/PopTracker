...
LIB_PNG_TAG="v1.6.58"
LIB_FREETYPE_TAG="VER-2-14-3"
LIB_OPENSSL_TAG="openssl-3.5.8"
...
if [ "$WITH_FREETYPE" ]; then
    ...
    if [ ! -d "$LIB_FREETYPE_DEST_DIR" ]; then
      ...
      cd "$LIB_FREETYPE_DEST_DIR" || exit 1
    else
      cd "$LIB_FREETYPE_DEST_DIR" || exit 1
      git pull
...
if [ ! -d "$LIB_SDL_IMAGE_DEST_DIR" ]; then
  ...
  cd "$LIB_SDL_IMAGE_DEST_DIR" || exit 1
else
  cd "$LIB_SDL_IMAGE_DEST_DIR" || exit 1
  git pull
...
if [ ! -d "$LIB_SDL_TTF_DEST_DIR" ]; then
  ...
  cd "$LIB_SDL_TTF_DEST_DIR" || exit 1
else
  cd "$LIB_SDL_TTF_DEST_DIR" || exit 1
  git pull
...
if [ ! -d "$LIB_OPENSSL_DEST_DIR" ]; then
  ...
  cd "$LIB_OPENSSL_DEST_DIR" || exit 1
else
  cd "$LIB_OPENSSL_DEST_DIR" || exit 1
  git pull
