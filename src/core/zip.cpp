...
std::vector< std::pair<Zip::EntryType,std::string> > Zip::list(const bool recursive)
{
    ...
    for (unsigned i = 0; i < mz_zip_reader_get_num_files(&_zip); i++) {
        ...
        if (mz_zip_reader_file_stat(&_zip, i, &st)) {
            ...
            if (name.empty())
                continue;
            const auto pSlash = name.find('/');
            const auto pBS = name.find('\\'); // work around bad zip
            if (pSlash != std::string::npos)
                _slashes |= Slashes::FORWARD;
            if (pBS != std::string::npos)
                _slashes |= Slashes::BACKWARD;
            if (!recursive && (
                    (pSlash != std::string::npos && pSlash != name.length()-1) ||
                    (pBS != std::string::npos && pBS != name.length()-1)))
                continue;
            res.emplace_back(
    ...
    for (unsigned i = 0; i < mz_zip_reader_get_num_files(&_zip); i++) {
        ...
        if (mz_zip_reader_file_stat(&_zip, i, &st)) {
            ...
            const auto pBS = name.find('\\');
            if (pBS < pSlash)
                pSlash = pBS;
            if (start.empty())
                start = name.substr(0, pSlash);
    ...
    if (!start.empty())
        res.emplace_back(EntryType::DIR, start);
