...
Packaging scripts also need a `sh` (e.g. bash) and `jq` to extract info from Meson introspection.
...
- install dependencies:
  ...
  mingw64/mingw-w64-x86_64-openssl p7zip meson jq
  ```
...
To run tests, `brew install googletest` and then `meson test -C build`.
